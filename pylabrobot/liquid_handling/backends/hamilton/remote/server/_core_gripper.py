...
class CoreGripperServerMixin:
    ...
    async def pick_up_core_gripper_tools(
        ...
        front_offset = (
            coordinate_from_proto(request.front_offset)
            if request.HasField("front_offset")
            else None
        )
        back_offset = (
            coordinate_from_proto(request.back_offset)
            if request.HasField("back_offset")
            else None
        )
    ...
    async def core_pick_up_resource(
        ...
        minimum_z_end = extract_optional_field(
            request, "minimum_z_position_at_the_command_end"
        )
